...
The Render service uses `render.yaml`, runs migrations before starting, binds Laravel to `0.0.0.0:$PORT`, and checks `/api/v1/health`. For Cloudflare R2, set `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_REGION=auto`, `R2_BUCKET`, and `R2_ENDPOINT=https://<account-id>.r2.cloudflarestorage.com` in the Render service environment. `R2_URL` is optional and should be the public bucket or custom-domain URL used to display uploaded media. The backend also accepts the equivalent `AWS_*` names. After changing environment variables, redeploy or clear/rebuild Laravel's config cache. For zero-downtime production releases, run migrations as a one-off release command or deployment pre-hook rather than concurrently from multiple web instances.